...
Mr Parmod is pursuing degree in theoretical Computer Science. He wants to be able to check
whether some string is part of a large regular expression or not. He has asked for help from you
guys. You have been given a regular expression and you have to construct a Nondeterministic
Finite Automaton (NFA). If string is part of regular expression, then you have to output matched,
else not matched. Regular expression will only contain numbers, | (OR), * (Star), + (Plus), e
(Epsilon) and small parenthesis.

• Numbers:- Numbers will be from 0 to 9.
• Star (*) :- The asterisk indicates zero or more occurrences of the preceding element.
• Plus (+) :- The plus sign indicates even number of occurrences of the preceding element
	and it is matched at least 4 times, but not more than 500 times. Example:- For regular
	expression (1+) string 1,11,111 and 1(502 times) will be rejected but 1111 and 111111
	will be accepted.

Constraints
• Length of every string will be less than 10000.
• Length of regular expression will be unbounded.
• In regular expression there will be only one level of parenthesis. Example :- (1(1|2)*|3)* is
not allowed but 1(1|2)*|3* is allowed.
...
NFA* NFA::plus()
{
	...
	NFA* f2 = Ncopy(th);
	NFA* f3 = Ncopy(th);
	n->allstate.insert(f1->allstate.begin(),f1->allstate.end());
	n->allstate.insert(f2->allstate.begin(),f2->allstate.end());
	n->allstate.insert(f3->allstate.begin(),f3->allstate.end());
